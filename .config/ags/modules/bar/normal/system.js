...
const BarBattery = () => Box({
    ...
    children: [
        // Battery display disabled for container
        Label({
            className: 'txt-smallie',
            label: 'N/A', // No battery in container
        }),
        Widget.Box({
            vpack: 'center',
            className: 'bar-batt',
            homogeneous: true,
            children: [
                MaterialIcon('power', 'small'), // Power icon instead of battery
            ],
        }),
    ]
});
...
const BatteryModule = () => Stack({
    ...
    setup: (stack) => Utils.timeout(10, () => {
        // Always show desktop mode for container (no battery)
        stack.shown = 'desktop';
    })
})
